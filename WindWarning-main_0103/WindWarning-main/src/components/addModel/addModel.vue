...
import { addModel, train } from '@/api/model'

const standPointList = ref(inject('standPointList'))
const algorithmStandRelateList = ref(inject('algorithmStandRelateList'))
...
// 监听 algorithmId 变化，更新 unselectedData
watch(
  () => stepData[0].algorithmId,
  (newAlgorithmId) => {
    if (newAlgorithmId) {
      // 根据 algorithmId 找到对应的测点列表
      const relatedPointIds = algorithmStandRelateList.value
        .filter((relate) => relate.algorithmId === parseInt(newAlgorithmId))
        .map((relate) => relate.standPointId)

      // 从 standPointList 中筛选出对应的测点
      stepData[1].unselectedData = standPointList.value.filter((point) => relatedPointIds.includes(point.pointId))
